...
public class LuceneContext {
	private static final Logger logger = LoggerFactory.getLogger(LuceneContext.class);
	...
	class IndexManager {
		...
		public Query createKeywordQuery(String keyword) {
			MultiFieldQueryParser parser = new MultiFieldQueryParser(LUCENE_VERSION, getVoPropertyNames(), new StandardAnalyzer(LUCENE_VERSION));
		...
		public PageInfo search(Query query, Sort sort, int page, int pageSize) {
			...
			try {
				...
				TopDocs docs = null;
				if (sort == null) {
					docs = searcher.search(query, 100000);
				} else {
					docs = searcher.search(query, 100000, sort);
				}
